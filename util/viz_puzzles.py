"""
Chess AC-JEPA — Puzzle Similarity Explorer

Encodes Lichess puzzles through the frozen AC-JEPA context encoder and
visualizes the embedding space as an interactive HTML scatter plot.
Similar tactical patterns should cluster together.

Usage:
  python util/viz_puzzles.py \\
    --ckpt checkpoints/checkpoint_epoch0005.pt \\
    --puzzles data/lichess_db_puzzle.csv \\
    --samples 3000

  # Re-use cached embeddings on subsequent runs:
  python util/viz_puzzles.py \\
    --ckpt checkpoints/checkpoint_epoch0005.pt \\
    --cache puzzle_cache.pkl \\
    --samples 3000

  # Or load from an existing puzzle_cache.pkl (skip --puzzles):
  python util/viz_puzzles.py \\
    --ckpt checkpoints/checkpoint_epoch0005.pt \\
    --cache puzzle_cache.pkl
...
THEME_PRIORITY = [
    ...
    "xRayAttack", "trappedPiece", "zugzwang", "quietMove", "backRankMate",
    "crushing", "advantage", "endgame", "middlegame", "opening",
]
...
def get_primary_theme(themes_str: str) -> str:
    themes = set(str(themes_str).strip().split())
    for t in THEME_PRIORITY:
        if t in themes:
            return t
    parts = str(themes_str).strip().split()
...
def load_puzzles(csv_path: str, num_samples: int, seed: int = 42) -> pd.DataFrame:
    ...
    df = pd.read_csv(csv_path, usecols=["PuzzleId", "FEN", "Moves", "Rating", "Themes", "GameUrl"])
...
# ─────────────────────────────────────────────────────────────────────────────
# Encoding — supports both ChessJEPA and ActionConditionedChessJEPA checkpoints
# ─────────────────────────────────────────────────────────────────────────────

@torch.no_grad()
def extract_puzzle_embeddings(
    ckpt_path: str,
    ...
    # Try AC model first; fall back to base JEPA if weights don't match
    try:
        ...
        print("  Loaded ActionConditionedChessJEPA")
    except Exception:
        model = ChessJEPA(
            encoder_kwargs=cfg.encoder_kwargs,
            predictor_kwargs=cfg.predictor_kwargs,
        ...
        model.load_state_dict(checkpoint["model"])
        print("  Loaded ChessJEPA (base model)")
    ...
    for row in tqdm(puzzles_df.itertuples(), total=len(puzzles_df)):
        ...
        puzzle_emb = latents.squeeze(0).mean(dim=(0, 1)).cpu().numpy()
...
def plot_puzzles(
    ...
    df: pd.DataFrame,
    method: str = "umap",
    out_file: str = "outputs/ac_puzzle_space.html",
):
    ...
    fig.update_layout(
        title=f"Chess AC-JEPA Puzzle Space ({method.upper()})",
...
function nextMove() {
  if (moveIdx < fenHistory.length - 1) { moveIdx++; chessBoard.position(fenHistory[moveIdx]); updateMoveLabel(); }
}
function prevMove() {
  if (moveIdx > 0) { moveIdx--; chessBoard.position(fenHistory[moveIdx]); updateMoveLabel(); }
}
function closeBoard() {
...
def main():
    parser = argparse.ArgumentParser(description="AC-JEPA Puzzle Similarity Explorer")
    parser.add_argument("--ckpt",    required=True,                        help="Checkpoint .pt file")
    parser.add_argument("--puzzles", default="data/lichess_db_puzzle.csv", help="Lichess puzzle CSV")
    ...
    parser.add_argument("--out",     default="outputs/ac_puzzle_space.html")
    parser.add_argument("--seed",    type=int, default=42)
    parser.add_argument("--cache",   default=None,
                        help="Cache file (.pkl). Saves after encoding; loads on next run to skip encoding.")
    ...
    if args.cache and os.path.exists(args.cache):
        ...
        if not os.path.exists(args.puzzles):
            print(f"ERROR: puzzle CSV not found at {args.puzzles}")
            print("Download it with:")
            print("  curl -O https://database.lichess.org/lichess_db_puzzle.csv.zst")
            print("  unzstd lichess_db_puzzle.csv.zst")
            sys.exit(1)
