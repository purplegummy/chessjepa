...
Your zarr store (chess_chunks.zarr) has this structure:
    └── boards : uint8 array of shape (N, 16, 17, 8, 8)
                 N chunks, each with 16 consecutive board positions
                 17 channels per board (12 piece planes + 4 castling + 1 en passant planes (no unused channel),
...
class ChessChunkDataset(Dataset):
    ...
    def __init__(self, zarr_path: str, split: str = "train", val_fraction: float = 0.05):
        ...
        # Zarr chunks are (128, 16, 17, 8, 8). If we ask for idx=0, it decompresses
    ...
    def __getitem__(self, idx: int) -> torch.Tensor:
        """
        Returns:
            boards : (16, 17, 8, 8) uint8 tensor — one game chunk
        """
...
class ActionChessChunkDataset(Dataset):
    ...
    ─────────────────────────────
        boards  : uint8    (N, T, 17, 8, 8)   — board state sequences
